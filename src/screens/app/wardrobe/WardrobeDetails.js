...
import {removeOutfitInCollectionApi} from '../../../services/AppApi';
...
export default function WardrobeDetails(props) {
  const data = props?.route?.params?.wardrobeDetail;
  const [loading, setLoading] = useState(false);
  const id = props?.route?.params?.id;
  const outfitId = data?._id;
  ...
  const handleRemoveOutfit = async () => {
    setSelected(false);
    setLoading(true);
    const response = await removeOutfitInCollectionApi(outfitId, id);
    console.log('responsefrooutfitremovefromwardrobe', response);
    if (response?.responseCode === 200) {
      setLoading(false);
      EventRegister.emit(registeredEvents.OUTFIT_REMOVE);
      setTimeout(() => {
      ...
      <ScrollView
        ...
        <View style={styles.mainView}>
          ...
          <TouchableOpacity
            ...
            <Image source={selected ? imagePath.liked : imagePath.likeE} />
          </TouchableOpacity>
