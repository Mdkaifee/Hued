...
export default function EditCategories2(props) {
  const {navigation} = props;
  // const colors = ['#000000', '#778899', '#FF0000', '#00FF00', '#0000FF'];
  const mainData = props?.route?.params?.outfitDetail;
  console.log(mainData, 'mainddddaaa');
  const selectedImages = mainData?.image[0];
  ...
  const [sliderValues, setSliderValues] = useState([mainData?.ageStart, mainData?.ageEnd]);
  const [priceValues, setPriceValues] = useState([mainData?.priceStart, mainData?.priceEnd]);
  ...
  const setupColorFetch = async () => {
    await GetPixelColor.init(selectedImages);
  };
  ...
  const handlePress = async event => {
    ...
    const color = await GetPixelColor.pickColorAt(
      (locationX * selectedImages?.width) / SCREEN_WIDTH,
      (locationY * selectedImages?.height) / (SCREEN_HEIGHT * 0.4),
    );
    console.log(color);
