...
export default function Categories2(props) {
  ...
  const setupColorFetch = async () => {
    if (Platform.OS === 'ios') {
      await GetPixelColor.init('file://' + selectedImages?.file);
    } else {
      await GetPixelColor.init(selectedImages?.data);
    }
  };
