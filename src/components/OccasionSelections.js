...
export default function OccasionSelections({onSelectedOccasion, occasion}) {
  const [selectedCategories, setSelectedCategories] = useState(occasion || []);
  const data = [
    {
      id: 1,
  ...
  const handleCategorySelect = name => {
    if (selectedCategories.includes(name)) {
      ...
      setSelectedCategories(prevSelected => {
        onSelectedOccasion([...prevSelected, name]);
